...
import { Heart, Eye, Share2, Flag, ShoppingBag, Clock, DollarSign, CheckCircle, ExternalLink } from "lucide-react";
...
const artworkData = {
  "1": {
    id: "1",
    title: "Neon Dreams",
    description: "A cyberpunk-inspired digital artwork exploring the intersection of technology and human consciousness. This piece represents the ethereal nature of digital dreams in a neon-lit urban landscape, where reality and virtuality blend seamlessly.",
    artist: {
      name: "CyberArt",
      username: "cyberart",
    ...
    price: 2.5,
    currency: "ETH",
    imageUrl: "https://images.unsplash.com/photo-1634017839464-5c339ebe3cb4?w=800&h=800&fit=crop",
...
export default function ArtworkDetail() {
  ...
  const confirmPurchase = () => {
    toast({
      ...
      description: "Your transaction is being processed. You'll receive a confirmation shortly.",
    });
    ...
    <Layout>
      <div className="min-h-screen py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <h1 className="text-3xl font-bold text-foreground mb-4">{artwork.title}</h1>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        <h3 className="font-semibold text-foreground">{artwork.artist.name}</h3>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-baseline space-x-2">
                        ...
                        <span className="text-lg text-muted-foreground">{artwork.currency}</span>
                    ...
                    <div className="flex space-x-3">
                      <Dialog open={isPurchaseDialogOpen} onOpenChange={setIsPurchaseDialogOpen}>
                        <DialogTrigger asChild>
                          <Button className="flex-1 bg-neon text-black hover:bg-neon/90" onClick={handlePurchase}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              You're about to purchase "{artwork.title}" by {artwork.artist.name}
                            </DialogDescription>
                          </DialogHeader>
                          <div className="space-y-4">
                            <div className="flex items-center space-x-4 p-4 border border-border rounded-lg">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-lg font-semibold text-foreground">{artwork.price} {artwork.currency}</p>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2 text-sm">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span>{(artwork.price * 0.025).toFixed(3)} {artwork.currency}</span>
                              </div>
                              <div className="flex justify-between font-semibold border-t border-border pt-2">
                                <span>Total</span>
                                <span>{(artwork.price * 1.025).toFixed(3)} {artwork.currency}</span>
                              </div>
                            </div>
                ...
                <div className="space-y-3">
                  {artwork.properties.map((property, index) => (
                    <div key={index} className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">{property.trait}</span>
                      <span className="text-sm font-medium text-foreground">{property.value}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <Button variant="ghost" size="sm" className="h-auto p-0">
                      <span className="text-sm font-mono">{artwork.contract.slice(0, 8)}...</span>
                      <ExternalLink className="w-3 h-3 ml-1" />
                    </Button>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Token ID</span>
                    <span className="text-sm font-mono text-foreground">{artwork.tokenId.slice(0, 8)}...</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Blockchain</span>
                    <span className="text-sm text-foreground">{artwork.blockchain}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Creator Royalties</span>
                    <span className="text-sm text-foreground">{artwork.royalties}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <div key={index} className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          {event.price && (
                            <span className="text-sm text-foreground">{event.price} ETH</span>
                          )}
                        </div>
          ...
          {/* Related Artworks */}
          <div className="mt-12">
            <h2 className="text-2xl font-bold text-foreground mb-6">More from this artist</h2>
